...
- **Tailwind CSS v4**: Configured via `@tailwindcss/vite` plugin. CSS variables for theming are in `assets/main.css`.
- **Dark/light mode**: Managed by `ThemeProvider` in `hooks/use-theme.tsx`. Uses a CSS class strategy (`.dark` on `<html>`). Always test both themes.

## Key Constraints

- **No Node.js APIs**: This runs in a browser context. Access Electron/Node via `window.electron` (exposed by preload).
